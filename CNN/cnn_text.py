...
def get_sentiment_data():
    df_sentiment = pd.read_csv('train.csv', encoding='utf-8')
    sentenses = df_sentiment['review'].values
    ...
    for line in df_sentiment.iterrows():
        # print('line')
        label, sentence = str(line[1]['label']), line[1]['review']
...
def test():
    ...
    textRNN = TextCNN(filter_sizes, num_filters, num_classes, learning_rate, batch_size, decay_steps, decay_rate,
                      MAX_SENTENCE_LENGTH, vocab_size, embedding_size, is_training)
    with tf.Session() as sess:
        ...
        for i in range(100):
            ...
            # input_x=np.zeros((batch_size,MAX_SENTENCE_LENGTH)) #[None, self.sequence_length]
